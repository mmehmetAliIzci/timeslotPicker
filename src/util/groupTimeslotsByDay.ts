...
export function groupTimeslotsByDay(timeslots: Array<ITimeslot>): Array<IGroupedTimeslots> {
    ...
    const groupedTimeslots: Array<{
        date: string;
        timeSlots: Array<IParsedTimeslot>;
    }> = [];

    sortedTimeslots.forEach((timeslot: ITimeslot) => {
        const parsedTimeslot: IParsedTimeslot = {
            start_time: new Date(timeslot.start_time),
            end_time: new Date(timeslot.end_time),
        };

        const dateKey = parsedTimeslot.start_time.toLocaleDateString(locale, {
            year: 'numeric',
            month: 'numeric',
            day: 'numeric',
        });

        const datePosition = groupedTimeslots.findIndex((date) => date.date === dateKey);
        if (datePosition > -1) {
